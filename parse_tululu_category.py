...
from tululu import download_book, download_image, get_response, parse_book_page
...
def main():
    parser = argparse.ArgumentParser(
        description="Load txt books from tululu.org from pages"
    )
    ...
    parser.add_argument(
        ...
        default="books",
        help=(
            "папка, в которую будут скачаны книги"
            ", изображения обложек и помещен "
            "json-файл с информацией о книгах."
        ),
    )
    parser.add_argument(
        "--skip_imgs", action="store_true", help="Не скачивать изображения обложек"
    )
    parser.add_argument("--skip_txt", action="store_true", help="Не скачивать книги")
    parser.add_argument(
        "--json_path",
        type=str,
        help="указать свой путь к *.json файлу с информацией о книгах",
    )
    ...
    if last_page:
        ...
        if first_page > max_page:
            print(
                "Первая страница, указанная при вызове скрипта больше"
                f" максимального числа страниц ({max_page})"
            )
            return
        if last_page > max_page:
            print(
                "Последняя страница, указанная при вызове скрипта, превышает "
    ...
    for num, book_url in enumerate(book_urls):
        ...
        try:
            ...
            print(
                f"Не удается найти главную страницу книги по адресу: {url}",
                file=sys.stderr,
            )
            ...
            if not skip_txt:
                try:
                    ...
                    filepath = os.path.join(
                        folder,
                        sanitize_filename(
                            f'{count}-я книга. {parsed_book["title"]}.txt'
                        ),
                    )
                    response = download_book(
                        "https://tululu.org/txt.php", params, filepath
                    )
            ...
            if not skip_imgs:
                try:
                    img_path = Path(
                        Path.cwd(), folder, "images", parsed_book["img_file_name"]
                    )
                    download_image(parsed_book["img_url"], img_path)
                except requests.HTTPError:
                    print(
                        "Не удается скачать изображение с URL:"
                        f' {parsed_book["img_url"]}',
                        file=sys.stderr,
                    )
                except requests.ConnectionError as e:
                    print("Проблема с интернет-соединением", file=sys.stderr)
